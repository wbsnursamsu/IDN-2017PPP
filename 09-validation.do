...
gen rpcexp_ppp_fooddefl = .
			
replace food_defl = food / (lasp_avgnatU_v5)
replace y_pcexp_fooddefl = ((food_defl + nfood)/hhsize) * 12
replace rpcexp_ppp_fooddefl = (y_pcexp_fooddefl/cpi2017/icp17/365)
gen poor_IPL215_fooddefl = rpcexp_ppp_fooddefl < 2.15
gen poor_IPL365_fooddefl = rpcexp_ppp_fooddefl < 3.65
gen poor_IPL685_fooddefl = rpcexp_ppp_fooddefl < 6.85			
...
label variable rpcexp_ppp_fooddefl "Real daily percapita consumption (st food + t nonfood)"
...
use "$gdOutput/FINAL - Exppl Spatially Adjusted - 2002-2022.dta", clear
collapse (mean) poor_IPL215 [w=weind], by(year urban)
tw (line poor_IPL215 year if urban == 0, lp(dash) lcol(maroon)) (line poor_IPL215 year if urban == 1, lp(dash) lcol(navy)), title("Standard IPL 2.15 USD PPP") legend(label(1 "IPL 2.15 USD PPP - Rural")  label(2 "IPL 2.15 USD PPP - Urban")) graphregion(color(white)) scale(0.75)
graph export "$gdOutput/Graphs/PA_Standard IPL Povrate.png", replace
	
** Quick check national & international U/R pl, by region
use "$gdOutput/FINAL - Exppl Spatially Adjusted - 2002-2022.dta", clear
collapse (mean) poor_IPL215 [w=weind], by(year region urban)
tw (line poor_IPL215 year if urban == 0, lp(dash) lcol(maroon)) (line poor_IPL215 year if urban == 1, lp(dash) lcol(navy)), by(region, title("Standard IPL 2.15 USD PPP") graphregion(color(white)) scale(0.75)) legend(label(1 "IPL 2.15 USD PPP - Rural") label(2 "IPL 2.15 USD PPP - Urban")) 
graph export "$gdOutput/Graphs/PA_Standard IPL Povrate - By Region.png", replace
...
forval i = 0/2  {
	...
	use "$gdOutput/FINAL - Exppl Spatially Adjusted - 2002-2022.dta", clear
	collapse (mean) poor_IPL`ipl'* [w=weind], by(year urban)
	tw (line poor_IPL`ipl' year, lp(solid) lcol(maroon)) (line poor_IPL`ipl'_fooddefl year, lp(dash_dot) lcol(maroon)), by(urban, scale(0.75) graphregion(color(white)) title("Standard vs Adjusted IPL `ipl2' - National")) legend(label(1 "`ipl2' IPL") label(2 "`ipl2' IPL - Spatially Deflated"))  xtitle("Year") ytitle("P0 Rate")
	graph export "$gdOutput/Graphs/PA_Standard vs Adjusted IPL `ipl2' P0 - National.png", replace
}
...
forval i = 0/2  {
	...
	gettoken ipl2 ipls2:ipls2
	use "$gdOutput/FINAL - Exppl Spatially Adjusted - 2002-2022.dta", clear
	collapse (mean) poor_IPL`ipl'* [w=weind], by(year region urban)
	tw (line poor_IPL`ipl' year if urban == 0, lp(dash_dot) lcol(maroon) lw(medthick)) (line poor_IPL`ipl' year if urban == 1, lp(dash_dot) lcol(navy) lw(medthick)) (line poor_IPL`ipl'_fooddefl year if urban == 0, lp(dot) lcol(maroon) lw(medthick)) (line poor_IPL`ipl'_fooddefl year if urban == 1, lp(dot) lcol(navy) lw(medthick)), by(region, scale(0.75) title("Standard vs Adjusted IPL `ipl2' - by Region") graphregion(color(white))) legend(label(1 "`ipl2' IPL - Rural") label(2 "`ipl2' IPL - Urban") label(3 "`ipl2' IPL - Rural, Spatially Deflated") label(4 "`ipl2' IPL - Urban, Spatially Deflated")) xtitle("Year") ytitle("P0 Rate")
	graph export "$gdOutput/Graphs/PA_Standard vs Adjusted IPL `ipl2' P0 - by Region All Graphs.png", replace
}

** All provinces
use "$gdOutput/FINAL - Exppl Spatially Adjusted - 2002-2022.dta", clear
collapse (mean) poor_IPL* [w=weind], by(year region)			
...
grc1leg2 g2.gph g3.gph, r(1) c(3) ycom xcom iscale(0.9) ysize(2) xsize(5) graphregion(color(white)) title("Regional Poverty Trends") scale(0.75) 
graph export "$gdOutput/Graphs/PA_Regional Poverty Trends_IPL.png", replace	
...
local graphs
local counter 1 2
local desc `" "Deflated (Total) 2.15 IPL" "Deflated (Food Only) 2.15 IPL" "'
foreach i in poor_IPL215 poor_IPL215_fooddefl {
...
grc1leg2 `graphs', r(1) c(2) ycom xcom iscale(0.75) ysize(3) xsize(4) graphregion(color(white)) title("Urban-Rural Poverty Trends") scale(0.75)
graph export "$gdOutput/Graphs/PA_Urban-Rural Poverty Trends_IPL.png", replace	
