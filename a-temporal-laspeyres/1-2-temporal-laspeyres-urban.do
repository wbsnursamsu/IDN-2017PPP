...
* Select categories
    ...
    merge m:1 code17 using "${gdTemp}/inclusion-item-temporal-urban.dta", nogen
...
* 1. population weights for province
    ...
    save "${gdTemp}/sus-cm-for-temporal-urban.dta", replace

* 2. Calculate weights - rural (plutocratic)    
    use "${gdTemp}/sus-cm-for-temporal-urban.dta", clear 
...
* 3. price data
    ...
    save "${gdTemp}/price-for-temporal-urban.dta", replace
    
    * 3a. rural price
    use "${gdTemp}/price-for-temporal-urban.dta", clear
    preserve
        keep if year==2017
...
//     replace p_ps = p0_ur if p_ps<(p0_ur/5) | p_ps>(p0_ur*5)
    ...
    /* SAVE */
    save "${gdOutput}/temporal-deflator-urban.dta", replace
