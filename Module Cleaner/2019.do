********************************************************************************
*	SUSENAS 2019 Consumption Module

clear all
cap log close
log using "${gdLog}/03-sus-cm-mar-2019.log", replace
...
use "${gdSUS}/MODULE/susenas19mar_41.dta",clear
...
use "${gdSUS}/MODULE/susenas19mar_42.dta",clear
rename renum urut
tostring urut, replace
destring(r101-r105), replace
...
merge m:1 code18 using "${gdTemp}/crosswalk-2018.dta", keepusing(code02 code04 code05 code06 code15 code17 code22 ditem_all item18 composite)
drop if _merge == 2
drop _merge
...
merge m:1 provcode kabcode using "${gdTemp}/region-crosswalk-2019.dta", keepusing(bps_code_all name_all)
drop if _merge==2
drop _merge
...
compress
save "$gdTemp/sus-cm-mar-2019.dta", replace
su shv_*