...
//     log using "${gdLog}/spatial-paasche-`t'.dta", replace
    ...
    * set price data
        rename kode code_c
        destring code_c, replace
        merge m:1 code_c using "${gdTemp}/shk-adjust-2019.dta", nogen
        drop if urut == ""
    ...
    * collapse to adjust with price survey and merge with price survey data
        ...
        merge m:1 provcode kode using "${gdPric}/shk-price-prov-bpscode-2019.dta", keepusing(p_ps) nogen 
...
//
    **# /* MIX HH IMPLICIT PRICE AND PRICE SURVEY */
        ...
        * save before collapse
        save "${gdTemp}/paache-deflator-`t'-mx.dta", replace
        
        /* province */
        preserve
            collapse (median) pdef_re pdef_hh [weight = popw] , by(prov) 
            replace pdef_re=1 if pdef_re==.
            replace pdef_hh=1 if pdef_hh==.
            la var pdef_re "Paasche spatial stratum level with regency UV"
            la var pdef_hh "Paasche spatial stratum level with HH UV"
            ...
            compress
            save "${gdOutput}/paache-deflator-pro-`t'-mx.dta", replace
        restore
...
//         save "${gdOutput}/paache-deflator-reg-`t'-mx.dta", replace
    ...
    **# /* MIX HH IMPLICIT PRICE AND PRICE SURVEY WITH RENT PRICE */
        ...
        * save before collapse
        save "${gdTemp}/paache-deflator-`t'-wr.dta", replace
        ...
        preserve
            ...
            compress
            save "${gdOutput}/paache-deflator-pro-`t'-wr.dta", replace
        restore        
